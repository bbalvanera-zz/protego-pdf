...
describe('when checking if is pdf document', () => {
    ...
    it('should return false if is not a pdf document', () => {
        let result = isPdfDocument('./tests/test-data/invalid.pdf');
        result = result.then(isOrNot => {
          console.log(`${isOrNot ? 'it is!' : 'I did not'}`);
          return isOrNot;
        });
        expect(result).resolves.toBe(false);
    });

    xit('should return true even if file is encrypted', () => {
        expect(isPdfDocument('./tests/test-data/test.encrypted[test].pdf')).resolves.toBe(true);
    });

    xit('should return false if file is corrupted', () => {
        expect(isPdfDocument('./tests/test-data/test.corrupted.pdf')).resolves.toBe(false);
    });

    xit('should fail if not a valid path', () => {
    ...
    xit('should fail if empty argument', () => {
        expect(isPdfDocument(null)).rejects.toBeDefined();
    });
});

xdescribe('when checking if is protected', () => {
...
xdescribe('when unlocking locked file', () => {
  let unlock: (source: string, target: string, password: string) => Promise<boolean>;
  beforeAll(() => {
    unlock = require('../lib/').unlock;
  });

  it('should unlock if correct password', () => {
    expect(unlock('./tests/test-data/test.encrypted[test].pdf', './tests/test-data/test.unlocked.pdf', 'abc')).resolves.toBe(true);
  });
});
